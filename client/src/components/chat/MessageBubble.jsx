...
export default function MessageBubble({ 
  ...
  return (
    <div className={`flex ${isMe ? 'flex-row-reverse' : 'flex-row'} items-end max-w-[85%] relative group max-w-full`}>
      ...
      {!isMe && (
        <motion.div 
          ...
          className="w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold uppercase shrink-0 border mb-1 mr-2.5 shadow-[0_4px_15px_rgba(0,0,0,0.3)] backdrop-blur-md"
        >
      ...
      <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-full relative`}>
        {!isMe && (
          <span 
        ...
        <div className="relative cursor-pointer max-w-full" onClick={() => onRipple(msg.id)}>
          ...
          <motion.div 
            ...
            <div className="relative z-10 w-full overflow-hidden">
              {msg.text && <div className="max-w-full break-words">{msg.text}</div>}
              {msg.fileUrl && (
                <div className="mt-2 relative">
                  {msg.fileType === 'image' ? (
                    ...
                    <a href={msg.fileUrl} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className={`flex items-center gap-2.5 p-3 rounded-xl border ${isMe ? 'bg-black/20 border-white/10 hover:bg-black/30' : 'bg-[rgba(255,255,255,0.05)] border-[rgba(255,255,255,0.08)] hover:bg-[rgba(255,255,255,0.1)]'} transition-colors text-sm backdrop-blur-md`}>
                      ...
                      <span className="font-medium underline-offset-4 hover:underline text-[13px] tracking-wide">Download File</span>
                    </a>
                  )}
                </div>
              )}
        ...
        <div className={`flex items-center gap-1.5 mt-1.5 mx-2 opacity-60 group-hover:opacity-100 transition-opacity ${isMe ? 'justify-end' : 'justify-start'}`}>
